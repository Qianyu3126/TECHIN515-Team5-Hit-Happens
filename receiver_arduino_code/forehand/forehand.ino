...
/*
 * forehand.ino  — Tennis Motion Analyzer receiver (FOREHAND model)
 *
 * One of three near-identical receiver sketches (forehand / backhand / serve).
 * Flash this one onto the receiver ESP32 when practicing forehands.
 ...
 * Plus the model library:  models/ei-0605-techin515-group-arduino-...zip
 *
...
void postToSupabase(const char* label, float confidence) {
  ...
  // ── PER-MODEL (3/3): label normalization (forehand) ──
  // model raw labels: "standard", "incomplete backswing", "incomplete followthough"
  String labelStr(label);
  String normalized;
  if (labelStr == "standard" || labelStr == "forehand_correct" || labelStr == "correct") {
    normalized = "forehand_correct";
  } else if (labelStr == "incomplete backswing" || labelStr == "incomplete_backswing" || labelStr == "forehand_incomplete_backswing") {
    normalized = "forehand_incomplete_backswing";
  } else if (labelStr == "incomplete followthough" || labelStr == "incomplete_followthough" || labelStr == "incomplete followthrough" || labelStr == "incomplete_followthrough" || labelStr == "forehand_incomplete_followthrough") {
  ...
  // ── PER-MODEL (3/3): feedback copy (forehand) ──
  const char* feedback = "";
...
void printResult(const char* label, float confidence) {
  ...
  String ls(label);
  if (ls == "standard" || ls == "forehand_correct" || ls == "correct") {
    Serial.println("# [OK] Correct form — solid forehand, keep it up!");
  } else if (ls == "incomplete backswing" || ls == "incomplete_backswing") {
  ...
  } else if (ls == "incomplete followthough" || ls == "incomplete_followthough" || ls == "incomplete followthrough" || ls == "incomplete_followthrough") {
